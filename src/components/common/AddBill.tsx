...
export const AddBill = ({ users, tripId }: { users: Users[], tripId: string }) => {
                ...
                <div className="flex w-full mt-7 justify-between gap-10">
                    <div className="w-full flex flex-col gap-4">
                        <div>
                            ...
                            <input type="text" id="first_name" name="billName" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="Bus,Comida ..." required />
                        </div>
                        <div>
                            ...
                            <select name="paidBy" className="border border-gray-300 rounded-lg p-2" id="" required>
                                {
                                    users.length > 0 && users.map((user) => {
                                        return (
                                            <option className="text-black p-2" key={user.id} value={user.id}>{user.name}</option>
                                        )
                                    })
                                }
                                ...
                                users.length > 0 && users.map((user) => {
                                    return (
                                        <div key={user.id} className="flex justify-start items-center">
                                            <input type="checkbox" className="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 " name="participants" value={user.id} />
                                            <label htmlFor="participants" className="ml-1">{user.name}</label>
                                        </div>
                                    )
                                })
                            }
